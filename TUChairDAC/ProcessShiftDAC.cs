...
namespace TUChairDAC
{
    ...
    {    
        ...
        public List<ProcessShiftVO> Bacode()
        {
            ...
            {
                SqlConnection conn = new SqlConnection(this.ConnectionString);
                string sql = @"select concat(upper(s.[Fact_Code]), '_',No) as BarCode, [No], s.[Item_Code] [Item_Code], [Item_Name],  s.[Fact_Code] [Fact_Code], [Fact_Name],[Fact_Type], [Insert_Date], [Qty],[Item_Size],[Item_Unit] ,[Stock_Other]
                                from [dbo].[Stock] s left join [dbo].[Item] i on s.[Item_Code] = i.[Item_Code] left join [dbo].[Factory] f on s.[Fact_Code] = f.[Fact_Code]
                                where [Insert_Date] is null and [Stock_Other] is null";
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
