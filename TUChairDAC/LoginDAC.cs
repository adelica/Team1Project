    ...
    public class LoginDAC : ConnectionAccess
    {
        ...
        public List<AuthorVO> GetAuthorInfo(int authorGroup_ID)
        {
            try
            {
                ...
                {
                    cmd.Connection = strConn;
                    cmd.CommandText = @"select AuthorGroup_ID, Program_ID, Program_Name, Program_order, Module_ID, Module_Name, Module_order, Method_Search, Method_Save, Method_New, Method_Delete, Method_Excel from vm_AuthorbyAuthGroup where AuthorGroup_ID = @authorGroup_ID";
