...
namespace TUChairDAC
{
    public class ProductClosingDAC : ConnectionAccess
    {
        ...
        public bool PCDeadline(string Primary, string Item, int Qty, string Modifier) //프로시저만들어야함
        {
            try
            {
        ...
        public List<ProductClosingVO> PDeadBinding() // 자재단가 관리 (전체)
        {
            try
            {
                SqlConnection conn = new SqlConnection(this.ConnectionString);
                string sql = @"select So_PurchaseOrder, c.Com_Name Com_Name, c.Com_Name d_Com_Name,  so.Item_Code Item_Code, i.Item_Code d_Item_Code, i.Item_name Item_name, 
		                             So_Qty, So_ProQty,So_Price Price, (So_Qty * So_Price) total,
		                               so.Modifier ,convert(nvarchar ,So_Duedate, 23) So_Duedate ,  convert(nvarchar ,GETDATE(), 23) So_OutDate 
                                from  SalesOrder so inner join SalesMaster sm on so.Sales_ID=sm.Sales_ID
                                		            inner join Company c on sm.Com_Code=c.Com_Code
                                		            inner join Item i on so.Item_Code = i.Item_Code
                                		            inner join Stock s on so.Item_Code = s.Item_Code and Fact_Code = 'WH_M'
                                where So_Qty  = So_ProQty and So_OutDate is not null and so_deadline = 'Y'";
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    conn.Open();
                    SqlDataReader reader = cmd.ExecuteReader();
        ...
        public bool PCDeadlineCancle(string Primary, string Item, int Qty, string Modifier) //프로시저만들어야함
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.Connection = new SqlConnection(this.ConnectionString);
                    cmd.CommandText = "SP_PDeadLineCancle";
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
